...
class Propietario extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nom_pro, telefono, direccion', 'required'),
			array('telefono, estado', 'numerical', 'integerOnly'=>true),
			array('nom_pro', 'length', 'max'=>50),
			array('direccion', 'length', 'max'=>90),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_propietario, nom_pro, telefono, direccion, estado', 'safe', 'on'=>'search'),
		);
	}
